...
While fully functional, the model still need improvement. Notably, numerical issues can be encounter for particular scenarios, particularly when controling the pH through HCO3 for this scenario, the reduction of the steepness for the answer of the dosage system seems to fix these numerical issue however the dosage system becomes less efficient. The innitial conditions if not consitent with the chemical equilibriums can also lead to numerical issues, e.g. Setting a very high pH a very high $CO_2$ and a very low $HCO_3^-$.
