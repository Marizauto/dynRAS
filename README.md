# dynRAS
The dynRAS model is a model developped to simulate recirculating aquaculture systems (RAS) with a particular emphasis on alkalinity and pH control. dIt integrates findings from prior research (referenced as Pedersen 2018 and Wik 2008) with new functionalities, including the incorporation of the carbonate system to accurately represent alkalinity and pH fluctuations. Developed within the scope of the RASTOOLS project, which seeks to delve into RAS dynamics, dynRAS simplifies its RAS components to include the fish tank, biofilter, and degasser, based on an experimental set-up by Jaffari et al. (2024). The model operates on Python 3.8 and necessitates the scipy, numpy, and math libraries.
...
* Biomass_function.py simulate fish removal using a threshold for the maximum biomass present in the system. The function is called every 14 days.
